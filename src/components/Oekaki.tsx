...
/* 낙서장(오에카키). progh2/mini-homepage 의 Oekaki.tsx 를 가져왔습니다 (MIT).
   이 저장소의 firebase.ts 에 맞춰 import 만 그대로 두고 본문은 손대지 않았습니다. */
...
export default function Oekaki() {
  ...
  useEffect(() => {
    ...
    return subscribeOekaki(60, viewer, setItems, error => {
      /* 대개 firestore.rules 에 낙서장 규칙이 아직 안 올라간 경우입니다. */
      console.error("[낙서장]", error.message);
      setFailed(true);
    });
  }, [viewer, me]);
  ...
  /* 아무것도 안 보이면 왜 안 되는지 알 길이 없습니다. 원인을 적어 둡니다. */
  if (failed) {
    return (
      <div className="cy-content-box">
        ...
        <div className="cy-gb-loading">
          낙서장을 불러오지 못했어요. 주인장이라면 Firebase 콘솔의 Firestore 규칙에
          낙서장 규칙(firestore.rules)이 올라가 있는지 확인해 주세요.
        </div>
      </div>
    );
  }
